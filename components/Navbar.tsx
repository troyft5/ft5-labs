...
export default function Navbar() {
  ...
  return (
    <nav className={`fixed top-0 w-full z-50 transition-all duration-300 ${scrolled ? 'shadow-lg shadow-black/40' : ''}`}
      ...
      {/* Announcement bar */}
      <div className="w-full py-2 px-4 text-center text-[11px] font-semibold" style={{ background: 'rgba(78,144,0,0.15)', borderBottom: '1px solid rgba(78,144,0,0.2)', color: '#8cd627' }}>
        <span className="mr-2 opacity-60">◆</span>
        New: April 2026 interchange rates are live — find out if yours changed.{' '}
        <Link href="/get-your-savings-estimate" className="underline underline-offset-2 hover:text-white transition-colors font-bold">Check my rates →</Link>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-6 py-3 flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-6">
          ...
          <Link href="/calculator" className="text-sm font-semibold text-slate-400 hover:text-white transition-colors">Calculator</Link>
          <Link href="/blog" className="text-sm font-semibold text-slate-400 hover:text-white transition-colors">Blog</Link>
          <Link href="/about-us" className="text-sm font-semibold text-slate-400 hover:text-white transition-colors">About</Link>
          <Link href="/contact-us" className="text-sm font-semibold text-slate-400 hover:text-white transition-colors">Contact</Link>
        </div>

        {/* Desktop CTAs */}
        <div className="hidden md:flex items-center gap-3">
          <a href="tel:6469417853" className="flex items-center gap-1.5 text-xs font-semibold text-slate-500 hover:text-slate-300 transition-colors">
            <Phone className="w-3.5 h-3.5" /> (646) 941-7853
          </a>
          <a href="https://app.fintech5group.com" target="_blank" rel="noopener noreferrer"
            className="px-4 py-2 text-xs font-bold text-slate-500 hover:text-slate-200 border border-white/10 hover:border-white/25 rounded-lg transition-all">
            Client Portal
          </a>
          <Link
            href="/get-your-savings-estimate"
            className="inline-flex items-center gap-2 px-5 py-2.5 text-sm font-black text-white rounded-xl transition-all hover:-translate-y-0.5 hover:shadow-xl"
            style={{ background: '#4e9000', boxShadow: '0 4px 16px rgba(78,144,0,0.4)' }}
          >
